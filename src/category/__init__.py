...
class Vector(list, Generic[_T]):
    ...
    def __add__(self, other: list[_T]) -> Vector[_T]:
        sequence = list(self)
        return __class__(sequence.__add__(other))

    def __setitem__(self, slice_: slice, item: Iterable[_T]) -> NoReturn:
    ...
    def remove(self, obj: Any) -> Vector[_T]:
        sequence = list(self)
        sequence.remove(obj)
        return __class__(sequence)

    def pop(self, index: int) -> Vector[_T]:
        sequence = list(self)
        sequence.pop(index)
        return __class__(sequence)
    ...
    def __reversed__(self) -> Vector[_T]:
        sequence = list(self)
        sequence.reverse()
        return __class__(sequence)
